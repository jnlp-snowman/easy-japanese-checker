...
class EasyJapanese2(EasyJapanese):
    """
    やさしい日本語の解析
    """
    def __init__(self, tagger_dir=""):
    ...
    def get_morph_type(self, unidic_id):
        ...
        row = db_session.query(EasyMorph2).filter(EasyMorph2.id == morph_id).one_or_none()
    ...
    def change_easy(self, unidic_id, surface):
        ...
        # 対象の語がデータベースに存在すれば、その行が戻り値となる。ない場合はNone
        row = db_session.query(EasyMorph2).filter(EasyMorph2.id == morph.id).one_or_none()
    ...
    def get_number_of_easy_morph(self):
        """
        """
        return db_session.query(EasyMorph2).count()

    def get_register_words(self):
        # 登録されたやさしい日本語を取得
        char_ids = db_session.query(EasyMorph2.id).all()
        char_ids = [char_id[0] for char_id in char_ids]

        # UniDicから品詞情報を取得
        unidic_ids = db_snow.db_session.query(db_snow.UniDic.ID).join(db_snow.CharMorph, db_snow.UniDic.ID==db_snow.CharMorph.UniDicId).filter(db_snow.CharMorph.CharId.in_(char_ids)).group_by(db_snow.CharMorph.CharId).all()
        unidic_ids = [unidic_id[0] for unidic_id in unidic_ids]
        # デバッグ用
        # print(len(unidic_ids))
